...
from modules.quantylab.rltrader import utils
...
mode = st.selectbox('학습 모드 설정', mode_list)
col1, col2 = st.columns(2)
data_upload = col1.file_uploader('학습데이터 업로드', type='csv')
data_v3 = col2.selectbox('또는 기본 제공 데이터 사용.', v3_data)
name = st.text_input('이름 설정', value='')
stock_code = name
rl_method = st.selectbox('강화학습방법 선택', rl_method_list)
net = st.selectbox('신경망 선택', net_list)
backend = "pytorch"
...
start_date = st.text_input('학습 시작일', value=start_date)
end_date = st.text_input('학습 종료일', value=end_date)
lr = st.number_input('learning rate', value=0.0001, step=0.0001,format="%.4f")
discount_factor = st.number_input('할인 요인', value=0.70, step=0.01, format="%.2f")
balance = st.number_input('balance', value=100000000)
risk = st.number_input('risk', value=.0, step=0.01, min_value=-1.0, max_value=1.0)

# argparse를 사용하여 파라미터 저장
args = argparse.Namespace(mode=mode, name=name, stock_code=stock_code,
                          rl_method=rl_method, net=net, backend=backend, start_date=start_date,
                          end_date=end_date, lr=lr, discount_factor=discount_factor,
                          balance=balance, risk=risk)

init_button = st.button('모델 학습 시작')

if init_button:
    # 학습기 파라미터 설정
    st.success('학습 시작. 좌측의 log page에서 학습 진행 상황을 확인하세요.')
